...
//message.reply("Operation Shadow Legacy ranked stats are currently unavailable. They will be up soon! For now you can use `!r6` , `!r6rankedsteel` and `!r6rankedvoid` .").then(message => {message.delete({timeout:30000})});
 ...
 (async () => {
	 ...
	 var gamesPlayed = Math.floor(json.seasons.grim_sky.regions.ncsa[0].wins + json.seasons.grim_sky.regions.ncsa[0].losses);
	 var kd = json.seasons.grim_sky.regions.ncsa[0].kills / json.seasons.grim_sky.regions.ncsa[0].deaths;
	 var wl = json.seasons.grim_sky.regions.ncsa[0].wins / json.seasons.grim_sky.regions.ncsa[0].losses;
	 var kdrounded = kd.toFixed(4);
	 var wlrounded = wl.toFixed(4);
	 var mmrNeeded = Math.floor(json.seasons.grim_sky.regions.ncsa[0].next_rank_mmr - json.seasons.grim_sky.regions.ncsa[0].mmr);
	 
	 let isrank = json.seasons.grim_sky.regions.ncsa[0];
 ...
 let rankstatsEmbed = new Discord.MessageEmbed()
  rankstatsEmbed.setAuthor(json.username + "'s R6 Grim Sky Ranked Stats", json.avatar_url_256)
  ...
  rankstatsEmbed.addField("Ranked Game Stats", "Wins: " + json.seasons.grim_sky.regions.ncsa[0].wins + '\n' + "Losses: " + json.seasons.grim_sky.regions.ncsa[0].losses + '\n' + "W/L: " + wlrounded + '\n' + "Games Played: " + gamesPlayed + '\n' + "Games Abandoned: " + json.seasons.grim_sky.regions.ncsa[0].abandons, true)
  rankstatsEmbed.addField("Rank", "Final Rank: " + json.seasons.grim_sky.regions.ncsa[0].rank_text + '\n' + "Highest Rank: " + json.seasons.grim_sky.regions.ncsa[0].max_rank_text + '\n' + "Final MMR: " + json.seasons.grim_sky.regions.ncsa[0].mmr + '\n' + "Last MMR Change: " + json.seasons.grim_sky.regions.ncsa[0].last_match_mmr_change + '\n' + "MMR required for next Rank: " + json.seasons.grim_sky.regions.ncsa[0].next_rank_mmr + '\n' + "MMR amount needed for next Rank: " + mmrNeeded, true)
  rankstatsEmbed.setFooter("Requested By: " + message.author.username, message.author.avatarURL())
  rankstatsEmbed.setTimestamp()
  message.reply("Here is the Operation Grim Sky ranked stats for " + json.username);
