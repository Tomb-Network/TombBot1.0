...
fs.readFile("./r6stats.json", (err, data) => {
	if (err) throw err;
	let linkedaccounts = JSON.parse(data)
	let userID = message.author.id;
	//console.log(linkedaccounts);
	
	//console.log(linkedaccounts[userID].account + " plat: " + linkedaccounts[userID].platform)


let platform = linkedaccounts[userID].platform;
let name = linkedaccounts[userID].account;

 message.reply(`Fetching Stats for your linked account ${name}, this may take a while.`).then(message => {message.delete({timeout:30000})});
